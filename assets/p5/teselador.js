...
class Tesela {
  ...
  show() {
    ...
    if (
      this.xpos > margin &&
      this.xpos < width - margin &&
      this.ypos > margin &&
      this.ypos < height - margin
    ) {
      return true;
    } else {
      return false;
...
function ubicarTeselas() {
  ...
  for (let y = 0; y < numY; y++) {
    ...
    let xOffset =
      (filaPar && teselaDerecha) || (!filaPar && !teselaDerecha) ? 0 : A / 2;
    for (let x = 0; x < numX; x++) {
      teselas[c].xpos = (A / 2) * x + xOffset;
      teselas[c].ypos =
        ((sqrt(3) * A) / 2) * y - (teselaDerecha ? 0 : (sqrt(3) * A) / 6);
  ...
  for(tesela of teselas){
    tesela.full = tesela.show(); // deja full verdadero si no está en el borde
  }
}
